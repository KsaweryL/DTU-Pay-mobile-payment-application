...
public class Event implements Serializable {
    ...
    private Object[] getArguments() {
        if (arguments == null) {
            ...
            while (currentClass != Event.class && currentClass != Object.class) {
                for (Field field : currentClass.getDeclaredFields()) {
                    if (field.isAnnotationPresent(EventArg.class)) {
            ...
            // Sort by the annotation order
            annotatedFields.sort(Comparator.comparingInt(f -> f.getAnnotation(EventArg.class).order()));
            ...
            for (Field field : annotatedFields) {
                ...
                try {
                    ...
                    throw new RuntimeException("Failed to access event field", e);
                }
            }
            arguments = args.isEmpty() ? null : args.toArray();
        }
        return arguments;
    }
	...
	public <T> T getArgument(int i, Class<T> cls) {
		...
		if (args == null || i < 0 || i >= args.length) {
			throw new IllegalArgumentException("Event argument index out of bounds: " + i);
		}
		...
		var gson = new Gson();
		var jsonString = gson.toJson(args[i]);
